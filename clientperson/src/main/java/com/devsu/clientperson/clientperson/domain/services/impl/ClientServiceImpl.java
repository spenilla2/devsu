...
public class ClientServiceImpl implements ClientService {
    ...
    public Optional<Client> updateClient(String identification, Client client) {
        Optional<Client> clientToUpdate = clientRepository.findByIdentification(identification).map(clientExist -> ClientEntityMapper.toClient(clientExist));
        if(clientToUpdate.isEmpty()){
            throw new ResourceNotFoundException("client", "identification", identification);
        }
        if(clientToUpdate.get().getIdentification() != client.getIdentification()){
            throw new BussinessException("The Client identification "+identification+" not match with the client to update");
        }
    ...
    public Optional<Client> deleteClientByIdentification(String identification) {
        ...
        try{
            clientRepository.deleteClientByIdentification(identification);
